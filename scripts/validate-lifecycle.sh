...
VELEIS_VERSION=1.8.11
VELEIS_IMAGE=docker.io/nyxmael/veleis:1.8.11
...
printf '%s\n' 'Veleis 1.8.11' >"$install_root/.veleis-installation"
printf '%s\n' 'name: veleis' >"$install_root/compose.yaml"

VELEIS_INSTALL_ROOT="$install_root" "$repository_root/veleis" upgrade 1.8.11 >"$temporary_directory/no-op.out"
...
grep -Fq 'downgrade from 1.8.11 to 1.7.0 is not supported' "$temporary_directory/downgrade.out"

exec 8>"$install_root/.maintenance.lock"
flock -n 8
if VELEIS_INSTALL_ROOT="$install_root" "$repository_root/veleis" upgrade 1.8.11 >"$temporary_directory/lock.out" 2>&1; then
...
if VELEIS_INSTALL_ROOT="$install_root" "$repository_root/veleis" upgrade 1.8.11 >"$temporary_directory/environment.out" 2>&1; then
...
if VELEIS_INSTALL_ROOT="$install_root" "$repository_root/veleis" upgrade 1.8.11 >"$temporary_directory/partial-profile.out" 2>&1; then
