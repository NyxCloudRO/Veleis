...
cat >"$temporary_directory/release.json" <<'JSON'
{
  ...
  "lifecycle_gated_upgrade_sources": ["2.0.0", "2.0.1", "2.0.2", "2.0.3", "2.0.4"],
...
if PATH="$fake_bin:$PATH" VELEIS_INSTALL_ROOT="$install_root" \
  ...
  "$temporary_directory/veleis-2.0.0" upgrade 2.0.5 >"$temporary_directory/old-cli.out" 2>&1; then
...
# the target 2.0.5 lifecycle tool has been installed.
...
if PATH="$fake_bin:$PATH" VELEIS_INSTALL_ROOT="$install_root" \
  ...
  VELEIS_TEST_RELEASE_METADATA="$temporary_directory/release.json" \
  VELEIS_TEST_DOCKER_MUTATIONS="$mutation_log" \
  "$temporary_directory/veleis-2.0.1" upgrade 2.0.5 >"$temporary_directory/old-cli-2.0.1.out" 2>&1; then
...
if PATH="$fake_bin:$PATH" VELEIS_INSTALL_ROOT="$install_root" \
  ...
  VELEIS_TEST_RELEASE_METADATA="$temporary_directory/release.json" \
  VELEIS_TEST_DOCKER_MUTATIONS="$mutation_log" \
  "$repository_root/veleis" upgrade 2.0.5 >"$temporary_directory/identity.out" 2>&1; then
