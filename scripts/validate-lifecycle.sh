...
VELEIS_VERSION=1.7.0
VELEIS_IMAGE=docker.io/nyxmael/veleis:1.7.0
...
ENVIRONMENT
printf '%s\n' 'Veleis 1.7.0' >"$install_root/.veleis-installation"
printf '%s\n' 'name: veleis' >"$install_root/compose.yaml"

VELEIS_INSTALL_ROOT="$install_root" "$repository_root/veleis" upgrade 1.7.0 >"$temporary_directory/no-op.out"
grep -Fq 'No backup, pull, migration, or restart was performed.' "$temporary_directory/no-op.out"

if VELEIS_INSTALL_ROOT="$install_root" "$repository_root/veleis" upgrade 1.6.9 >"$temporary_directory/downgrade.out" 2>&1; then
  echo 'downgrade was accepted' >&2
  exit 1
fi
grep -Fq 'downgrade from 1.7.0 to 1.6.9 is not supported' "$temporary_directory/downgrade.out"

exec 8>"$install_root/.maintenance.lock"
flock -n 8
if VELEIS_INSTALL_ROOT="$install_root" "$repository_root/veleis" upgrade 1.7.0 >"$temporary_directory/lock.out" 2>&1; then
...
if VELEIS_INSTALL_ROOT="$install_root" "$repository_root/veleis" upgrade 1.7.0 >"$temporary_directory/environment.out" 2>&1; then
