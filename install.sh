...
readonly RELEASE_METADATA_SHA256="166297a4f55fd012733662befe0848457f4aa183fca953ae59c59e9cb9d50746"
...
case "${ID:-}" in
  debian | ubuntu) ;;
  *) fail "unsupported operating system '${ID:-unknown}'; Veleis 1.8.3 supports tested Debian and Ubuntu releases" ;;
esac
...
case "$architecture" in
  ...
  *) fail "unsupported architecture '$architecture'; Veleis 1.8.3 supports linux/amd64" ;;
esac
...
jq -e '.product == "Veleis" and .version == "1.8.3" and .schema == 33 and .backup_format_version == 1' "$TEMPORARY_DIRECTORY/release.json" >/dev/null || fail "release metadata is incompatible"
