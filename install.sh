...
readonly RELEASE_METADATA_SHA256="145cd49fbdd6d2a21e90c089f2a7a94f0992a868e1bf9fe0ac78bd790103fb8b"
...
case "${ID:-}" in
  debian | ubuntu) ;;
  *) fail "unsupported operating system '${ID:-unknown}'; Veleis 1.8.0 supports tested Debian and Ubuntu releases" ;;
esac
...
case "$architecture" in
  ...
  *) fail "unsupported architecture '$architecture'; Veleis 1.8.0 supports linux/amd64" ;;
esac
...
jq -e '.product == "Veleis" and .version == "1.8.0" and .schema == 33 and .backup_format_version == 1' "$TEMPORARY_DIRECTORY/release.json" >/dev/null || fail "release metadata is incompatible"
