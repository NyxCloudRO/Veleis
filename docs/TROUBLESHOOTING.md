...
Error contains `unsupported operating system`. Veleis 1.8.12 is accepted only on
...
```bash
sudo docker pull nyxmael/veleis:1.8.12
...
and later fix a query path that could scan and sort the complete historical
...
readiness, sufficient free space, schema 45 in a clean migration state, and no
