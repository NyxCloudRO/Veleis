...
No. Veleis 2.0.4 performs one bounded scalar numeric-OID `GET` per probe
...
Not in 2.0.4. The accepted public image is linux/amd64 only.

## Which Linux releases are supported?

Veleis 2.0.4 installation is supported and validated on Ubuntu 24.04 LTS,
...
directly to 2.0.4. Veleis 2.0.0 through 2.0.3 require the documented
lifecycle-tool refresh first. Every path uses a mandatory backup and
immutable-digest verification. On 2.0.4, the same command is a no-op. Targets must explicitly
...
The supported 2.0.4 installer uses its generated certificate.
