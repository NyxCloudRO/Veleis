...
No. Veleis 2.0.0 performs one bounded scalar numeric-OID `GET` per probe
...
Not in 2.0.0. The accepted public image is linux/amd64 only.

## Which Linux releases are supported?

Veleis 2.0.0 installation is supported and validated on Ubuntu 24.04 LTS,
...
1.8.5, 1.8.6, 1.8.7, 1.8.8, 1.8.9, 1.8.10, 1.8.11, and 1.8.12 upgrade to the current 2.0.0
stable release after a mandatory backup and immutable-digest verification. On
2.0.0, the same command is a no-op. Targets must explicitly
...
The supported 2.0.0 installer uses its generated certificate.
