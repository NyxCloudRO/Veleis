...
No. Veleis 1.8.8 performs one bounded scalar numeric-OID `GET` per probe
...
Not in 1.8.8. The accepted public image is linux/amd64 only.

## Which Linux releases are supported?

Veleis 1.8.8 installation is supported and validated on Ubuntu 24.04 LTS,
...
1.8.5, 1.8.6, and 1.8.7 upgrade to the current 1.8.8 stable release after a mandatory backup and
immutable-digest verification. On 1.8.8, the same command is a no-op. Targets must explicitly
...
The supported 1.8.8 installer uses its generated certificate.
