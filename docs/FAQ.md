...
No. Veleis 1.8.12 performs one bounded scalar numeric-OID `GET` per probe
...
Not in 1.8.12. The accepted public image is linux/amd64 only.

## Which Linux releases are supported?

Veleis 1.8.12 installation is supported and validated on Ubuntu 24.04 LTS,
...
1.8.5, 1.8.6, 1.8.7, 1.8.8, 1.8.9, 1.8.10, and 1.8.11 upgrade to the current 1.8.12
stable release after a mandatory backup and immutable-digest verification. On
1.8.12, the same command is a no-op. Targets must explicitly
...
The supported 1.8.12 installer uses its generated certificate.
