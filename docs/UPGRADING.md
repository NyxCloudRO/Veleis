...
Veleis 2.0.6 is the current stable release. Veleis 1.7.1 and 1.8.0 through
1.8.12 are explicit direct upgrade sources. Veleis 2.0.0 through 2.0.5 are
...
The exact target form is `sudo veleis upgrade 2.0.6`. Schema 54 is current.
The 2.0.5 and 2.0.4 paths keep schema 54 and replace only the application
...
On 2.0.6, `sudo veleis upgrade` and `sudo veleis upgrade 2.0.6` are safe
no-ops: they create no backup, pull no image, run no migration, and restart no
service. A downgrade from 2.0.6 remains rejected.

## PostgreSQL memory profile

New 2.0.6 installations select a managed database profile from the effective
