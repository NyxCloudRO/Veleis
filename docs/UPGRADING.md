...
Veleis 1.8.3 is the current stable release. Veleis 1.7.1, 1.8.0, 1.8.1, and
1.8.2 are explicit upgrade sources. `sudo veleis upgrade` discovers 1.8.3; the
exact target form is `sudo veleis upgrade 1.8.3`. Schema 33 remains current (1.7.1
advances from 32).
...
On 1.8.3, `sudo veleis upgrade` and `sudo veleis upgrade 1.8.3` are safe
no-ops: they create no backup, pull no image, run no migration, and restart no
service. A 1.8.3 downgrade remains rejected.
