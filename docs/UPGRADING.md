...
Veleis 2.0.5 is the current stable release. Veleis 1.7.1 and 1.8.0 through
1.8.12 are explicit direct upgrade sources. Veleis 2.0.0 through 2.0.4 are
...
The exact target form is `sudo veleis upgrade 2.0.5`. Schema 54 is current.
The 2.0.4 path keeps schema 54 and replaces only the application image. The
2.0.3 path applies migration 54; earlier supported sources advance through
...
On 2.0.5, `sudo veleis upgrade` and `sudo veleis upgrade 2.0.5` are safe
no-ops: they create no backup, pull no image, run no migration, and restart no
service. A downgrade from 2.0.5 remains rejected.

## PostgreSQL memory profile

New 2.0.5 installations select a managed database profile from the effective
