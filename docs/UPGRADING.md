...
Veleis 2.0.7 is the current stable release. Veleis 1.7.1, 1.8.0 through
1.8.12, and 2.0.6 are explicit direct upgrade sources. Veleis 2.0.0 through 2.0.5 are
lifecycle-gated sources: refresh the lifecycle tooling before the upgrade so
the schema-54 compatibility contract and current
...
The exact target form is `sudo veleis upgrade 2.0.7`. Schema 54 is current.
The 2.0.6, 2.0.5, and 2.0.4 paths keep schema 54 and replace only the application
image. The 2.0.3 path applies migration 54; earlier supported sources advance through
every required migration through schema 54 in order.
...
On 2.0.7, `sudo veleis upgrade` and `sudo veleis upgrade 2.0.7` are safe
no-ops: they create no backup, pull no image, run no migration, and restart no
service. A downgrade from 2.0.7 remains rejected.

## PostgreSQL memory profile

New 2.0.7 installations select a managed database profile from the effective
