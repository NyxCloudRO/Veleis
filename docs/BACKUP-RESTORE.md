...
```bash
sha256sum --check veleis-backup-1.8.10-YYYYMMDDTHHMMSSZ.tar.gz.sha256
```
...
```bash
sudo veleis restore /secure/path/veleis-backup-1.8.10-YYYYMMDDTHHMMSSZ.tar.gz --force
```
...
restore secrets and files, recreate services, validate schema 43, and wait for
HTTPS readiness.
...
Veleis 1.8.10/schema 43, and complete same-version restore coverage on
linux/amd64. Supported 1.7.1 and 1.8.0 through 1.8.9 to 1.8.10 upgrades preserve populated state,
