...
```bash
sha256sum --check veleis-backup-2.0.1-YYYYMMDDTHHMMSSZ.tar.gz.sha256
```
...
```bash
sudo veleis restore /secure/path/veleis-backup-2.0.1-YYYYMMDDTHHMMSSZ.tar.gz --force
```
...
restore secrets and files, recreate services, validate schema 50, and wait for
HTTPS readiness.
...
Veleis 2.0.1/schema 50, and complete same-version restore coverage on
linux/amd64. Supported 1.7.1 and 1.8.0 through 1.8.12, plus 2.0.0, to 2.0.1 upgrades
