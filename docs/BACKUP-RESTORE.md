...
```bash
sha256sum --check veleis-backup-2.0.0-YYYYMMDDTHHMMSSZ.tar.gz.sha256
```
...
```bash
sudo veleis restore /secure/path/veleis-backup-2.0.0-YYYYMMDDTHHMMSSZ.tar.gz --force
```
...
Veleis 2.0.0/schema 50, and complete same-version restore coverage on
linux/amd64. Supported 1.7.1 and 1.8.0 through 1.8.12 to 2.0.0 upgrades
