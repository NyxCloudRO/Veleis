...
```bash
sha256sum --check veleis-backup-1.8.9-YYYYMMDDTHHMMSSZ.tar.gz.sha256
```
...
```bash
sudo veleis restore /secure/path/veleis-backup-1.8.9-YYYYMMDDTHHMMSSZ.tar.gz --force
```
...
Veleis 1.8.9/schema 40, and complete same-version restore coverage on
linux/amd64. Supported 1.7.1 and 1.8.0 through 1.8.8 to 1.8.9 upgrades preserve populated state,
