...
```bash
sha256sum --check veleis-backup-1.8.6-YYYYMMDDTHHMMSSZ.tar.gz.sha256
```
...
```bash
sudo veleis restore /secure/path/veleis-backup-1.8.6-YYYYMMDDTHHMMSSZ.tar.gz --force
```
...
restore secrets and files, recreate services, validate schema 36, and wait for
HTTPS readiness.
...
Veleis 1.8.6/schema 36, and complete same-version restore coverage on
linux/amd64. Supported 1.7.1/1.8.0/1.8.1/1.8.2/1.8.3/1.8.4/1.8.5 to 1.8.6 upgrades preserve populated state,
