...
[![Current release](https://img.shields.io/badge/release-v2.0.2-14b8a6)](https://github.com/NyxCloudRO/Veleis/releases/tag/v2.0.2)
...
Current stable release: **Veleis 2.0.2** · Schema 51 · linux/amd64
...
`nyxmael/veleis:2.0.2` image, detects effective cgroup memory, provisions a
...
Veleis 2.0.2 is currently validated on the following amd64 platforms:
...
| Anomaly Detection     | Adaptive, explainable CPU, memory, and network anomaly detection with confidence, evidence, and bounded learning states                                                   |
| Capacity Intelligence | Supported host-filesystem forecasts with threshold timing, confidence, data quality, and explicit unsupported or insufficient-history states                              |
| Docker                | Automatic discovery plus engines, containers, runtime state, metrics, events, images, volumes, networks, and availability incidents                                       |
| Proxmox               | Read-only nodes, VMs, LXCs, storage, networks, inventory metadata, and relationships                                                                                      |
| Infrastructure        | Normalized Discovery inventory, Storage/NAS visibility, history, changes, search, hierarchy, topology, and explicit cross-provider identity trust                         |
...
- Adaptive Anomaly Detection for CPU, memory, and network signals using robust
  statistics, with explainable evidence, confidence, and bounded learning states.
- Capacity Intelligence for supported host filesystems, with forecast confidence,
  data quality, threshold timing, and explicit unsupported states.
...
- Bounded indexed latest CPU/memory retrieval for Agents, with propagated
  request cancellation, debounced search, coalesced live refresh, retained
  rows, and recoverable timeout/error states.
- Read-only automatic Docker discovery and GET-only Proxmox observations.
...
- Probe deletion retires associated alert work without fabricating recovery or
  discarding delivered notification history.
- Consistent accessible Veleis confirmation dialogs for destructive record actions.
...
`sudo veleis upgrade 2.0.2`. Existing 2.0.0 and 2.0.1 installations must first
refresh the lifecycle command as documented below. Installations created before
...
- Certificate Intelligence begins collecting identity and change history only
  after schema 36 is installed and a TLS Certificate probe runs; it cannot
  reconstruct observations from before the upgrade.
